...
const HeaderLogos = () => {
  ...
  const settings = {
    ...
    responsive: [
      ...
      },
      {
        breakpoint: 576,
        settings: {
          slidesToShow: 3, // 1 slide vizibil pe ecrane foarte mici
          slidesToScroll: 1,
        },
      },
    ],
  ...
  return (
    <Slider className="navbar_brand_mobil float-start" {...settings}>
      {parteneri.map((val) => (
        <Image
          key={val.id} // Presupunând că `val.id` este unic
          src={val?.logo?.finalUri}
          alt={val?.denumireBrand}
          width={50} // Dimensiuni ajustate pentru o vizualizare mai bună
          height={50}
          objectFit="contain"
